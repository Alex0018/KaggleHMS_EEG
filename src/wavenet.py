...
# https://www.kaggle.com/code/cdeotte/wavenet-starter-lb-0-52/notebook
# https://www.kaggle.com/code/alejopaullier/hms-wavenet-pytorch-train
...
class SequentialWaveNet(nn.Module):
    def __init__(self):
        super(SequentialWaveNet, self).__init__()
        self.model = WaveNet()
        self.global_avg_pooling = nn.AdaptiveAvgPool1d(1)
        self.dropout = 0.0
        self.head = nn.Sequential(
            # nn.Linear(320, 64),      # for regional averages
            nn.Linear(256, 64),      # for original double banana
            nn.BatchNorm1d(64),
            nn.ReLU(),
    ...
    def forward_regavg(self, input: torch.Tensor):

        x = []
        for i in range(5):
            x1 = self.model(input[:, :, i:i+1])
